...
// src/utils/ActivityIndicator.ts
var ActivityIndicator = class {
  constructor() {
    this.spinner = null;
    this.timerId = null;
  }
  add() {
    ...
    for (const delay of bounceDelays) {
      ...
      (_a3 = this.spinner) == null ? void 0 : _a3.appendChild(bounce);
    }
...
// src/open_ai/OpenAiClientLibrary.ts
...
var OpenAiApi = class {
  constructor(plugin) {
    ...
    this.chat = async (promptOrMessages, model, systemMessage, maxTokens, maxOutgoingCharacters, baseURL, apiKey, organization) => {
      var _a3, _b;
      const openai = new OpenAI({
        apiKey: apiKey != null ? apiKey : this.plugin.settings.defaultApiKey,
      ...
      try {
        const completion = await openai.chat.completions.create({
          messages,
          model: model != null ? model : this.plugin.settings.defaultModel,
          max_completion_tokens: maxTokens != null ? maxTokens : this.plugin.settings.defaultMaxNumTokens
        });
        ...
        if (((_a3 = import_obsidian.Platform.isDesktop && this.plugin.settings.displayTokenUsageDesktop) != null ? _a3 : false) || ((_b = import_obsidian.Platform.isMobile && this.plugin.settings.displayTokenUsageMobile) != null ? _b : false)) {
          const { usage } = completion;
          if (usage) {
            const displayMessage = `${this.plugin.APP_ABBREVIARTION}:
...
Character count: ${characterCount.toString()}`;
            new import_obsidian.Notice(displayMessage, 8e3);
          }
        }
...
var promotionalLinks = (containerEl) => {
...
var OWlSettingTab = class extends import_obsidian2.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    new import_obsidian2.Setting(containerEl).setName("Endpoint").setDesc(
    ...
    ).setClass("ait-settings").addText((text) => {
      text.setValue(this.plugin.settings.defaultEndpoint).onChange(async (value) => {
        this.plugin.settings.defaultEndpoint = value;
        await this.plugin.saveSettings();
      });
    });
    new import_obsidian2.Setting(containerEl).setName("API Key").setDesc("The API Key for accessing the AI endpoint.").setClass("ait-settings").addText((text) => {
    ...
    new import_obsidian2.Setting(containerEl).setName("Model").setDesc("Name of language model to use.").setClass("ait-settings").addText((text) => {
      text.setValue(this.plugin.settings.defaultModel).onChange(async (value) => {
        this.plugin.settings.defaultModel = value;
        await this.plugin.saveSettings();
      });
    });
    new import_obsidian2.Setting(containerEl).setName("Max tokens per request").setDesc(
    ...
    new import_obsidian2.Setting(containerEl).setName("Max characters per outgoing request").setDesc(
      ...
      text.setValue(
        ...
        this.plugin.settings.defaultMaxOutgoingCharacters = Number.parseInt(value);
        await this.plugin.saveSettings();
      });
    });
    new import_obsidian2.Setting(containerEl).setName("System message").setDesc(
      "Default system message sent with each prompt. This message is used to initialize the conversation."
    ).setClass("ait-settings").addTextArea((textEl) => {
    ...
    new import_obsidian2.Setting(containerEl).setName("Debugging").setHeading();
    new import_obsidian2.Setting(containerEl).setName("Debug to the console").setDesc("Dumps detailed into to the console for debugging.").addToggle((cb) => {
      var _a3;
      cb.setValue((_a3 = this.plugin.settings.debugToConsole) != null ? _a3 : false);
      cb.onChange(async (value) => {
        this.plugin.settings.debugToConsole = value;
        await this.plugin.saveSettings();
      });
    });
    new import_obsidian2.Setting(containerEl).setName("Display token usage on desktop").setDesc(
      ...
      var _a3;
      cb.setValue((_a3 = this.plugin.settings.displayTokenUsageDesktop) != null ? _a3 : false);
      cb.onChange(async (value) => {
        this.plugin.settings.displayTokenUsageDesktop = value;
        await this.plugin.saveSettings();
      });
    });
    new import_obsidian2.Setting(containerEl).setName("Display token usage on mobile").setDesc(
      "Displays the  number of tokens used a notification on the mobile."
    ).addToggle((cb) => {
      var _a3;
      cb.setValue((_a3 = this.plugin.settings.displayTokenUsageMobile) != null ? _a3 : false);
      cb.onChange(async (value) => {
...
// node_modules/templater-obsidian/src/core/functions/internal_functions/InternalModule.ts
var InternalModule = class {
  constructor(plugin) {
    this.plugin = plugin;
    __publicField(this, "static_functions", /* @__PURE__ */ new Map());
    __publicField(this, "dynamic_functions", /* @__PURE__ */ new Map());
    __publicField(this, "config");
    __publicField(this, "static_object");
  }
  getName() {
    return this.name;
  }
...
// src/templater/InternalModuleAit.ts
var InternalModuleAit = class extends InternalModule {
  constructor() {
    super(...arguments);
  ...
  generate_selection_or_nearest() {
    return () => {
      var _a3;
      const active_editor = (_a3 = this.plugin) == null ? void 0 : _a3.app.workspace.activeEditor;
      if (!(active_editor == null ? void 0 : active_editor.editor)) {
  ...
  async generate_content_without_properties() {
    var _a3;
    if (this.config.target_file instanceof import_obsidian3.TFile) {
      const file = this.config.target_file;
      const fileContents = await ((_a3 = this.plugin) == null ? void 0 : _a3.app.vault.read(file));
      return fileContents ? fileContents.replace(/^---\n([\s\S]*?)\n---\n/, "") : "";
    }
    throw new Error("target_file is not a TFile");
  }
  generate_run_chat() {
    return async (promptOrMessages, model, systemMessage, maxTokens, maxOutgoingCharacters, baseURL, apiKey, organization) => {
      var _a3, _b;
      const spinner = new ActivityIndicator_default();
      spinner.add();
      try {
        const result = (_b = await ((_a3 = this.plugin) == null ? void 0 : _a3.openAiApi.chat(
          promptOrMessages,
          model,
          systemMessage,
          maxTokens,
...
// src/templater/Templater.ts
var initializeTemplaterInternalModule = async (plugin) => {
  return new Promise((resolve, reject) => {
    let retries = 30;
    ...
    around(plugin.app.plugins, {
      enablePlugin(oldMethod) {
        return async function(pluginId) {
          if (pluginId === "templater-obsidian") {
            setTimeout(async () => {
              var _a3;
              if ((_a3 = window.ait) == null ? void 0 : _a3.plugin)
                window.ait.plugin.internalModuleAit = await initializeTemplaterInternalModule(window.ait.plugin);
            }, 1e3);
          }
      ...
      disablePlugin(oldMethod) {
        return async function(pluginId) {
          var _a3;
          if (pluginId === "templater-obsidian") {
            if ((_a3 = window.ait) == null ? void 0 : _a3.plugin) window.ait.plugin.internalModuleAit = null;
          }
          return oldMethod.call(this, pluginId);
        };
      }
...
// src/main.ts
var AitPlugin = class extends import_obsidian4.Plugin {
  constructor() {
    super(...arguments);
    this.APP_NAME = this.manifest.name;
    this.APP_ID = this.manifest.id;
    this.APP_ABBREVIARTION = "AIT";
